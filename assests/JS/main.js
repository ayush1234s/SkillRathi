 ...
 const closeCourseAccessBtn = document.getElementById('closeCourseAccessBtn');
 const courseAccessLoginBtn = document.getElementById('courseAccessLoginBtn');
 const courseLinks = document.querySelectorAll('.course-link');
 
 // Course Links Click Handler
 courseLinks.forEach(link => {
     link.addEventListener('click', (e) => {
         e.preventDefault();
         const currentUser = JSON.parse(localStorage.getItem('currentUser'));
         const coursePath = link.getAttribute('data-course');
         
         if (currentUser) {
             // Redirect to course page
             window.location.href = `courses/${coursePath}.html`;
         } else {
             // Show access modal
             courseAccessModal.classList.remove('hidden');
         }
     });
 });
 
 // Close Course Access Modal
 closeCourseAccessBtn.addEventListener('click', () => {
     courseAccessModal.classList.add('hidden');
 });
 
 courseAccessModal.addEventListener('click', (e) => {
     if (e.target === courseAccessModal) {
         courseAccessModal.classList.add('hidden');
     }
 });
 
 // Course Access Login Button
 courseAccessLoginBtn.addEventListener('click', () => {
     courseAccessModal.classList.add('hidden');
     toggleAuthModal();
 });
